error id: file://<WORKSPACE>/hw/spinal/MY_Prj/pipCPU/pipCPU.scala:spinal/core/UInt#asBits().
file://<WORKSPACE>/hw/spinal/MY_Prj/pipCPU/pipCPU.scala
empty definition using pc, found symbol in pc: 
found definition using semanticdb; symbol spinal/core/UInt#asBits().
...
class EX_stage(
    ...
    val csr_fun = stage.down(CSR_fun)
  ...
  switch(fun){
    ...
    is(ECALL ){csr_valid:=True;rd_valid:=False;csr_fun:=1}
    is(MRET  ){csr_valid:=True;rd_valid:=False;csr_fun:=2}
    is(CSRRW ){csr_valid:=True;rd_valid:=False;csr_fun:=3}
    is(CSRRS ){csr_valid:=True;rd_valid:=False;csr_fun:=4}
    is(CSRRC ){csr_valid:=True;rd_valid:=False;csr_fun:=5}
    is(CSRRWI){csr_valid:=True;rd_valid:=False;csr_fun:=6}
    is(CSRRSI){csr_valid:=True;rd_valid:=False;csr_fun:=7}
    is(CSRRCI){csr_valid:=True;rd_valid:=False;csr_fun:=8}
...
//执行控制部分
    ...
    val CSR_fun =Payload(UInt(12 bits))
...
//写回控制部分
    ...
    val WB = new pip.Ctrl(5){
      ...
      when(RD_sel=/=0){gpr(RD_sel.resize(4 bits)):=csr_rdata}

      switch(CSR_fun.asBits.a@@){
      is(1 ){
      ...
      is(3 ){csr_wdata:=gpr_rdata}
      is(4 ){csr_wdata:=gpr_rdata | csr_rdata}
      is(5 ){csr_wdata:=csr_rdata & ~ gpr_rdata}
      is(6){csr_wdata:=CSR_rs1_imm.asBits.resized}
      is(7){csr_wdata:=(gpr_rdata.asUInt | CSR_rs1_imm.resize(32 bits)).asBits}
      is(8){csr_wdata:=csr_rdata & ~ (CSR_rs1_imm.resize(32 bits).asBits)}
      }
      }
    }
